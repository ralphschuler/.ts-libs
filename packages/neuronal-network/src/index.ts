...
export class Neuron<T> {
  ...
  activate(input: T): T {
    ...
    for (const connection of this.incomingConnections) {
      sum += connection.weight * (connection.neuron.activate(input) as any);
    }
...
export class Network<T> {
  ...
  activate(input: T[]): T[] {
    ...
    return this.outputLayer.neurons.map((neuron: Neuron<T>, index: number) =>
      neuron.activate(output[index]),
    );
  }
  ...
  propagate(error: T[]): void {
    ...
    this.outputLayer.neurons.map((neuron: Neuron<T>, index: number) =>
      neuron.propagate(error[index]),
    );
  }
