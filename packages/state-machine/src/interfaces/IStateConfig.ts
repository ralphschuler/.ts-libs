 ...
 */
export interface IStateConfig<States extends Record<string, any>, Events extends Record<string, any>, Payloads extends Merge<[States, Events]>, SubMachineEvents extends Record<keyof SubMachineEvents, any>> {
  /** Action executed when entering this state. */
  onEntry?: Action<Payloads[keyof Payloads]>;

  /** Action executed when exiting this state. */
  onExit?: Action<Payloads[keyof Payloads]>;
  ...
  on?: {
      [event in keyof Events | keyof SubMachineEvents]?: ITransition<States, Payloads[keyof Payloads]>;
  };
