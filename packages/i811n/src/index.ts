...
class Translator<T extends Translations> {
    ...
    translate<K extends TranslationKeys<T[LanguageCode]>>(key: K, variables?: Record<string, string>): string {
        const translations = this.manager.getTranslations();
        let translation = translations[key] || `[${key}] not found in ${this.manager.getCurrentLanguage()}`;
        if (variables) {
...
const translationManager = new TranslationManager<Translations>('en', translations);
