...
export function ConfigProperty(type: string, defaultValue: any) {
  return function (target: any, propertyKey: any): void {
    ...
    const properties = Reflect.getMetadata('configProperties', constructor) || new Map();
...
export function InterfaceMethod() {
  return function (target: any, propertyKey: any): void {
    const constructor = target.constructor;
    const methods = Reflect.getMetadata('interfaceMethods', constructor) || new Map();
...
function validateAndFreezeConfig(config: UnifiedConfig<any>, mixins: Function[]) {
  ...
  mixins.forEach((Mixin) => {
    ...
    const expectedConfig: Map<string, any> = Reflect.getMetadata('configProperties', Mixin) || new Map();
    expectedConfig.forEach((value, key) => {
      ...
      if (value.type !== typeof (config as any)[key]) {
        throw new Error(`Type mismatch for ${key} in ${className}: expected ${value.type}, got ${typeof (config as any)[key]}`);
      }
    });
  });
}
...
type UnifiedConfig<T extends any[]> = { [K in keyof T]: ExtractConfig<T[K]> }[number];

export async function createClass<T extends Array<new (config: any) => any>>(...mixins: T): Promise<new (config: UnifiedConfig<T>) => any> {
  return class {
